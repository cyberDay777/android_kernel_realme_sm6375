...
unsigned int omh_debug = 1;
unsigned int omh_time_flag = 0;
...
{
	unsigned long long time_ms_cnt = 0;
	...
	for (i = 0; i < MAX_LIGHTS; i++) {
		if (lights_health_para[i][1] == LIGHTS_ON && lights_health_para[i][2] == OFF) {
			...
			lights_health_para[i][7] = lights_health_para[i][3];
			omh_time_flag = 0;
			...
			lights_health_para[i][0] > LIGHTS_GREEN ? "LIGHTS_BLUE" : \
			lights_health_para[i][0] ==  LIGHTS_GREEN ? "LIGHTS_GREEN" : "LIGHTS_RED");
			time_ms_cnt = lights_health_para[i][4] - lights_health_para[i][7];
		} else if (lights_health_para[i][1] == LIGHTS_ON && lights_health_para[i][2] == ON) {
			lights_health_para[i][2] = ON;
			lights_health_para[i][3] =  ktime_to_ms(ktime_get());
			time_ms_cnt = lights_health_para[i][3] - lights_health_para[i][7];
			lights_health_para[i][7] = lights_health_para[i][3];
			omh_time_flag = 1;

			OMH_DEBUG("%s ALWAYS ON",
			lights_health_para[i][0] > LIGHTS_GREEN ? "LIGHTS_BLUE" : \
			lights_health_para[i][0] ==  LIGHTS_GREEN ? "LIGHTS_GREEN" : "LIGHTS_RED");
		} else {
			omh_time_flag = 0;
        ...
        if (omh_time_flag) {
			if (lights_health_para[i][5] < MAX_CNT) {
				lights_health_para[i][5] += time_ms_cnt/CHG_TIME;
			} else {
			...
			/*power_cnt = lights_health_para[i][7] * 4 * (time_ms_cnt / CHG_TIME);*/
			lights_health_para[i][8] = power_cnt;

			OMH_DEBUG("debug : [%d][%d][%d]",
				lights_health_para[i][5], time_ms_cnt, lights_health_para[i][7]);
		}
	}
...
int oplus_misc_healthinfo(int type, int para, int bright)
{
	...
	OMH_DEBUG("oplus_misc_healthinfo : [%d][%d][%d]", type, para, bright);
...
static int proc_misc_healthinfo_read(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < MAX_LIGHTS; i++) {
		seq_printf(s, "name:light_%s,brightness:%lld,time(s):%lld\n",
			lights_health_para[i][0] > LIGHTS_GREEN ? "blue" : \
			lights_health_para[i][0] ==  LIGHTS_GREEN ? "green" : "red", \
			lights_health_para[i][6], lights_health_para[i][5]);
		OMH_DEBUG("name:light[%d],brightness:%d,time(ms):%d", i, lights_health_para[i][6], lights_health_para[i][5]);
	}
